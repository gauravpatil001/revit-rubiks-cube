...
# Rotate around left face outward normal (-X) through project internal origin.
origin = XYZ(0, 0, 0)
...
cubie_size_ft = 1.0
left_layer_x_ft = -cubie_size_ft  # -1.0 ft
x_tolerance = 0.01
...
left_layer = [
    ...
    if abs(point.X - left_layer_x_ft) <= x_tolerance
]

if not left_layer:
    forms.alert(
        "Left layer could not be identified at X = -1.0 ft.",
        exitscript=True,
    )

if len(left_layer) != 9:
    forms.alert(
        "Expected 9 cubies in left layer at X = -1.0 ft, found {}. "
        "Check cubie placement/origins.".format(len(left_layer)),
        exitscript=True,
    )

with revit.Transaction("Rotate Left Face"):
    for cubey in left_layer:
