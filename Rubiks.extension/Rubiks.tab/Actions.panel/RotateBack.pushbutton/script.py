...
# Rotate around back face outward normal (-Y) through project internal origin.
origin = XYZ(0, 0, 0)
...
cubie_size_ft = 1.0
back_layer_y_ft = -cubie_size_ft  # -1.0 ft
y_tolerance = 0.01
...
back_layer = [
    ...
    if abs(point.Y - back_layer_y_ft) <= y_tolerance
]

if not back_layer:
    forms.alert(
        "Back layer could not be identified at Y = -1.0 ft.",
        exitscript=True,
    )

if len(back_layer) != 9:
    forms.alert(
        "Expected 9 cubies in back layer at Y = -1.0 ft, found {}. "
        "Check cubie placement/origins.".format(len(back_layer)),
        exitscript=True,
    )
