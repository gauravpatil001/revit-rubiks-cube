...
cubie_size_ft = 1.0
bottom_layer_z_ft = -cubie_size_ft
z_tolerance = 0.05
...
if len(bottom_layer) != 9:
    forms.alert(
        "Expected 9 cubies in bottom layer at Z = -1.0 ft, found {}.".format(len(bottom_layer)),
        exitscript=True,
    )
...
if failed_ids:
    forms.alert("Bottom rotation failed for ids: {}".format(", ".join(str(x) for x in failed_ids)))
