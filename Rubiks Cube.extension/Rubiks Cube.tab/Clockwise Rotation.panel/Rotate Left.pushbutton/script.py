...
left_layer = [elem for elem, point in cubies_with_points if abs(point.X - left_layer_x_ft) <= x_tolerance]

if len(left_layer) != 9:
    forms.alert(
        "Expected 9 cubies in left layer at X = -1.0 ft, found {}.".format(len(left_layer)),
        exitscript=True,
    )
...
if failed_ids:
    forms.alert("Left rotation failed for ids: {}".format(", ".join(str(x) for x in failed_ids)))
