...
cubie_size_ft = 1.0
front_layer_y_ft = cubie_size_ft
y_tolerance = 0.05
...
front_layer = [elem for elem, point in cubies_with_points if abs(point.Y - front_layer_y_ft) <= y_tolerance]

if len(front_layer) != 9:
    forms.alert(
        "Expected 9 cubies in front layer at Y = +1.0 ft, found {}.".format(len(front_layer)),
        exitscript=True,
    )
...
if failed_ids:
    forms.alert("Front rotation failed for ids: {}".format(", ".join(str(x) for x in failed_ids)))
