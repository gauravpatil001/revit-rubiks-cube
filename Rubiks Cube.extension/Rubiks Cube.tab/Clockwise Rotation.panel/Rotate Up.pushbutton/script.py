...
if len(top_layer) != 9:
    forms.alert(
        "Expected 9 cubies in top layer at Z = +1.0 ft, found {}.".format(len(top_layer)),
        exitscript=True,
    )
...
if failed_ids:
    forms.alert("Top rotation failed for ids: {}".format(", ".join(str(x) for x in failed_ids)))
